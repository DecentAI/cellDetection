...
class UNet3Plus(nn.Module):
    def __init__(self, in_channels=3, n_classes=1, bilinear=True, feature_scale=4,
    ...
    def forward(self, inputs):
        ...
        #     x = F.interpolate(x, size=(h, (w//32)*32), mode='bilinear', align_corners=True)
        # if(h%32!=0):
        #     #case when width is not divisible by 8
        #     x = F.interpolate(x, size=((h//32)*32, w), mode='bilinear', align_corners=True)
        x = F.interpolate(x, size=((h//32)*32, (w//32)*32), mode='bilinear', align_corners=True)
