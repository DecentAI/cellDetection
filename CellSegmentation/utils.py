...
<<<<<<< HEAD
    ...
    for idx, (x, y) in enumerate(loader):
        ...
        for i in range(len(preds)):
            ...
            # cv2.imwrite(f"{folder}{idx}_{i}_input.png", bgr)
            # torchvision.utils.save_image(x[i], f"{folder}original_{idx}_{i}.png")
            ...
            # cv2.imwrite(f"{folder}{idx}_{i}_mask.png", y_tmp)

            pd = cv2.cvtColor(preds[i].cpu().numpy(), cv2.COLOR_GRAY2BGR)*255
            # torchvision.utils.save_image(preds[i], f"{folder}{idx}_{i}_pred.png")
            ...
            # torchvision.utils.save_image(y[i], f"{folder}{idx}_{i}.png")
        if(counter == 100):
            break
        else:
            counter +=1
